...
def _get_feed(rec_sys, user_id: str, n: int = 30) -> pd.DataFrame:
    ...
    # 3. Trả về kết quả Real-time hoặc Hybrid
    if recent_msids:
        return rec_sys.recommend_realtime(user_id, recent_listened_msids=recent_msids, n=n)
...
def render_interactive_tab(rec_sys, user_input, n_recs, db_path="./mappings.db"):
    ...
    *Hệ thống sẽ thay đổi gợi ý ngay lập tức dựa trên mỗi cú click của bạn. 
    ...
    placeholder_image = "https://media.istockphoto.com/id/134119615/vi/anh/b%E1%BA%A3n-ghi-vinyl.jpg?b=1&s=612x612&w=0&k=20&c=br5IW88j4z_-9GK_laewFTnxo9pIsbqKEkwbqEViMfY="
    with st.spinner("Đang cập nhật luồng nhạc..."):
        feed_df = _get_feed(rec_sys, user_input, n=n_recs)

    if not feed_df.empty:
        # TĂNG LÊN 5 CỘT (Thay vì 3 cột như cũ để card nhỏ lại)
        num_cols = 5 
        ...
        for idx, row in feed_df.iterrows():
            with cols[idx % num_cols]:
                with st.container(border=True):
                    ...
                    # 3. Nút bấm nhỏ gọn
                    button_key = f"btn_{row['track_name']}_{idx}"
