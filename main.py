...
class AdvancedHybridRecommender:
    ...
    def recommend_hybrid(self, user_id_str, n=10, liked_tracks=None, liked_artists=None, filter_listened=True, use_mmr=True, artist_limit=2, trending_boost=0.0, content_alpha=0.25):
        ...
        if use_mmr and len(rec_ids) >= n: final_ids, final_scores = self._mmr_rerank(rec_ids, rec_scores, n=n)
        else: final_ids, final_scores = rec_ids[:n], rec_scores[:n]
    ...
    def recommend_inclusive(self, user_id_str, n_warm: int = 7, n_cold: int = 3) -> pd.DataFrame:
    ...
    def recommend_cold_content(self, user_id_str=None, text_query=None, n=10):
        if text_query:
    ...
    def generate_playlist(self, user_id_str, seed_track_names=None, playlist_name='Gợi ý dành riêng cho bạn', n_songs=15):
    ...
    def recommend_trending(self, user_id_str=None, n=10, personal_weight=0.5):
    ...
    def recommend_similar_users(self, user_id_str, n=10, k_users=20, filter_listened=True):
        tier, uid = self._get_user_tier(user_id_str)
        if uid is None: return self.popular_items(n)
    ...
    def recommend_discovery(self, user_id_str, n=10, serendipity=0.3):
        tier, uid = self._get_user_tier(user_id_str)
        if uid is None: return self.popular_items(n)
        self._ensure_user_index()
    ...
    def recommend_next_in_session(self, session_msids, n=10, decay=0.8, filter_session=True):
    ...
    def recommend_by_artist(self, artist_name, n=10, expand=True, k_similar_artists=5):
