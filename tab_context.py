...
def render_context_tab(rec_sys, user_input, n_recs):
    ...
    with tab1:
        ...
        if st.button("🔍 Phân tích nội dung", key="btn_new"):
            df = rec_sys.recommend_similar_to_new_item(t_name, a_name, n=n_recs)
            if not df.empty:
    ...
    with tab2:
        ...
        if seed_query:
            ...
            if search_results.empty:
                ...
                if st.button("🚀 Sinh Playlist", use_container_width=True):
                    # Tách lấy tên bài hát từ chuỗi đã chọn
                    # (Giả sử định dạng là: Tên Bài - Tên Ca sĩ)
                    chosen_track = selected_version.split(" - ")[0]
                    
                    with st.spinner(f"Đang sinh nhạc dựa trên: {selected_version}..."):
                        df = rec_sys.generate_playlist(user_input, seed_track_names=[chosen_track], n_songs=n_recs)
